...
const loginUserCtrl = asyncHandler(async (req, res) => {
    ...
    if(findUser && (await findUser.isPasswordMatched(password))){
        ...
        throw new Error('Invaild Credentis')
    }
})
const loginAdmin = asyncHandler(async (req, res) => {
    ...
    if(findAdmin && (await findAdmin.isPasswordMatched(password))){
        ...
        res.json({
            ...
            token: generateToken.generateToken(findAdmin?._id)
        })
    }else {
        throw new Error('Invaild Credentis')
    }
...
const forgotPasswordToken = asyncHandler(async (req, res) =>{
    ...
    try{
        ...
        await user.save()
        const resetURL = `Hi , Please follow this link to reset Your Password , This link is valid till 10min from now. <a href='http://localhost:5000/api/user/reset-password/${token}'> Click Here </> `
        const { data } = {
            to: email,
            text: "Hey User",
...
const getWishlist = asyncHandler(async (req, res) => {
    const { _id } = req.user;
    try {
        const findUserr = await User.findById(_id).populate("wishlist");
        console.log(_id)
      res.json(findUserr);
    } catch (error) {
      throw new Error(error);
